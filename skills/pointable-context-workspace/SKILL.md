...
8. For revision qualification, leave one card open, change the selected file or one bounded relation, confirm `内容已更新`, then click `刷新内容`. Verify that the same card DOM stays visible at the same position, preserves scroll and disclosure state, displays a finite type-prioritized diff before the P-C model when projected fields changed, keeps an ordinary unrefreshed card quiet, opens no browser, and adds no Chat Turn.
