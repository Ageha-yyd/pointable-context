...
- Treat workspace discovery and usable identity capacity as separate bounds. Small workspaces may keep the full compatible file surface; large workspaces must keep the complete development-context surface rather than an arbitrary first-N slice. If the development surface or discovery bound is exceeded, stop fail-closed and report it; never silently omit later source, test, document, configuration, or checked artifact identities.
...
- At a stable milestone, `object-review --review-file <absolute-json> --json` may measure only the terms explicitly declared as needed for resume, handoff, decision, status, verification, or understanding. The strict review document has exactly `schemaVersion`, `milestoneKey`, and `needs`; each need has exactly `term`, `expectedEntityType`, and `needKind`. The JSON token for an understanding need is exactly `"understand"` (the earlier `"understanding"` spelling is accepted only as a compatibility alias and is normalized to `"understand"`). The command resolves identity only against the current runtime-validated workspace + task-local surface and reports available, missing, ambiguous, and type-mismatch results. It must not read Provider detail, persist the review, infer undeclared needs, or treat the result as human-efficiency evidence.
- Keep `object-review` as the non-persisting diagnostic. Only when the current long task is explicitly opted in and has reached a real stable milestone may `milestone-observe --review-file <absolute-json> --json` append that same declared-need measurement to the private Companion ledger. The event stores only term/milestone/context fingerprints, status/type/source-layer signals, timestamps, and aggregate lifecycle/capacity counts. It must not store raw terms, milestone names, Chat, Provider facts, file content, workspace paths, or task/thread IDs; it does not mutate objects, cards, or Chat.
- Use `milestone-summary --json` to inspect only the current task/workspace context across multiple recorded milestones. Interpret recurring fingerprints and aggregate omission/ambiguity/type-mismatch as object-curation calibration signals, never as a Dashboard, inferred object request, human-efficiency result, or permission to synthesize records.
- The `milestone-observe` response may also return `feedback` for only the raw terms already present in that explicit current review. Require `measurement=ephemeral_current_review_feedback` and `persisted=false`. Interpret transition labels only against the immediately previous observation for the same term and context: `gap → available` is `recovered` once, then a later `available → available` is `stable_available`; historical gaps must not make recovery sticky. `recurring_gap` requires the immediately previous state to be the same gap and may return `review_registration`, but that is a prompt to apply the existing stable-milestone judgment—not permission to create or update an object, artifact, card, or Chat Turn automatically. The ledger and summary must never persist that feedback or its raw terms, and another task/workspace context starts with no inherited feedback history.
...
6. If this bounded long task is explicitly using dynamic objects, run `object-list --json`, inspect its capacity warning, then apply only the sparse lifecycle mutations justified by the current milestone. At a stable milestone, run read-only `object-audit --json` to distinguish active partials, stable overlaps, terminal archive candidates, unmatched terminal records, and ambiguous matches. When the user or Agent has an explicit bounded list of terms that were actually needed at that milestone, run `object-review --review-file <absolute-json> --json`; do not invent a list by mining Chat. If and only if the task is also opted into longitudinal observation, use `milestone-observe` instead to append the private digest-only event and inspect its ephemeral current-review feedback, then use `milestone-summary` for current-context trends. Run `object-archive --json` only after a stable artifact has passed its checker and the corresponding task-local object is terminal.
...
node $companion object-review --review-file 'C:\absolute\temporary-review.json' --json
node $companion milestone-observe --review-file 'C:\absolute\temporary-review.json' --json
node $companion milestone-summary --json
...
- If a milestone observation context changes while the review is running, the private ledger is corrupt, its hash chain fails, its path enters the workspace, or its 512-event/4-MiB bound is reached, fail closed. Never repair it by dropping events, widening the path, or writing raw review input.
