...
- The `milestone-observe` response may also return `feedback` for only the raw terms already present in that explicit current review. Require `measurement=ephemeral_current_review_feedback` and `persisted=false`. Treat `first_observation`, `stable_available`, `new_gap`, `changed_gap`, and `recovered` as bounded status only. `recurring_gap` may return `review_registration`, but that is a prompt to apply the existing stable-milestone judgment—not permission to create or update an object, artifact, card, or Chat Turn automatically. The ledger and summary must never persist that feedback or its raw terms, and another task/workspace context starts with no inherited feedback history.
