...
After a stable milestone or before handoff, run the bundled read-only coverage gate. If the bounded task also uses task-local objects and the current milestone has an explicit list of terms that were actually needed, use the workspace companion's `object-review` separately to measure those declared needs against the current identity surface. Do not derive that list by scanning Chat and do not confuse this review with structural coverage. When the same explicitly opted-in long task needs longitudinal curation calibration, `milestone-observe` may persist the review only as a private digest/status/type/source/count event and `milestone-summary` may aggregate the current context. These observations must contain no raw term, milestone name, Chat, Provider fact, file content, workspace path, or task/thread ID; they are neither stable artifacts nor human-efficiency evidence.
