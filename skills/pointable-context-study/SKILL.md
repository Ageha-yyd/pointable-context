...
8. If and only if the release declares the native scripted runner qualified, launch the unscored `TRAIN-1` as ordinary Codex turns. Keep the runner process alive. When it emits `state: "awaiting_native_task"`, read only its control fields and call the Codex Desktop native task-navigation tool with the emitted `threadId`; do not search or click a sidebar row and do not create a replacement task. Continue polling the runner until TRAIN-1 terminates. Do not accept a browser or full-overlay transcript imitation as the practice surface, and do not use a model to answer it.
...
The native-navigation step is mechanical coordination only. Never summarize, interpret, answer, or expose the scripted training content. If native navigation fails or the participant closes the task before answering, leave the package and checkpoint unchanged and report the fixed error; do not synthesize another task.
