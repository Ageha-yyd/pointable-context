...
| Runner reports `awaiting_native_task`, but no sidebar task appears | App Server task creation completed, but sidebar discovery is not the activation contract. | Keep the runner alive and have the setup/session coordinator open the exact emitted `threadId` with Codex native task navigation. | Do not create a replacement task or retry by clicking the sidebar. |
| `study_v2_desktop_navigation_unverified` or task activation timeout | The native task ID was not opened in the qualified Codex Desktop task surface before the bounded timeout. | Preserve the package/checkpoint, rerun through the designated coordinator, and report the fixed error if native navigation still fails. | Do not fall back to a browser, a fake transcript, or a live model. |
