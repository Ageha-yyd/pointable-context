...
import { checkContextMilestoneArtifacts } from "./context-artifact-check.js";
...
function usage(): string {
  return "Usage: pointable-context-artifact-check --workspace-root <path> [--json]";
}
...
if (parsed === undefined) {
  ...
  const result = await checkContextMilestoneArtifacts(parsed.workspaceRoot ?? "");
  if (parsed.json) {
    process.stdout.write(`${JSON.stringify(result)}\n`);
  } else if (result.valid) {
    process.stdout.write(`Pointable Context milestone artifacts valid: ${result.artifacts.length}\n`);
  } else {
    process.stdout.write(`Pointable Context milestone artifacts invalid: ${result.issues.length}\n`);
