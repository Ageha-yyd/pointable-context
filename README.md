...
The historical v2.21 `OpenAI.Codex 26.814.5517.0` / executable `151.0.7922.137` record passed all four automatic and all ten evidence-bound manual gates against renderer digest `616008d2…5154`; it remains historical evidence for v2.21 only. The v2.22 Task Object Registry and card-local text-selection bundle (`6a0aaa90…c6a2`) also independently passed automatic 4/4 plus manual 10/10 and remains historical evidence for that exact bundle. v2.24 remained unqualified after manual acceptance exposed an offscreen-anchor orphan-card defect. v2.25 closed an un-moved card correctly, but refresh qualification then exposed that a deliberately dragged card still disappeared after new Chat output and that alias-heavy catalogs could omit later objects. v2.26 (`f570c4de…843c`) pinned a user-moved card, allocated one primary annotation term per unambiguous object before aliases, and independently passed automatic 4/4 plus manual 10/10 on exact Codex package `26.814.5517.0`. v2.27 (`fbc4d463…c278`) added the read-only curation audit and passed automatic host gates 4/4 with all manual gates pending. v2.28 added explicit milestone object review; v2.29 added reliable test-execution events and its `77d7051e…3814` bundle passed automatic 4/4. The current v2.30 product slice adds the private multi-milestone observation ledger and has bundle digest `b659f894…6209` on exact Codex package `26.818.5229.0`; the running Desktop was not launched with the loopback CDP endpoint, so its automatic gate is truthfully `unavailable` and all ten manual gates remain pending. v2.30 is not yet build-qualified. No earlier automatic or manual evidence is inherited. Run the read-only inspector after relaunching that exact host with the loopback endpoint:
