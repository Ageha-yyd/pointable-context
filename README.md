...
The developer-only [Recovery Observation Prototype](docs/recovery-observation-prototype.md) prepares that later observation without mounting new native UI. It validates privacy-bounded relative-time events and derives active recovery, card-dwell, navigation, Chat Turn, interaction, wrong-object, refresh, and lookup-failure metrics. Three deterministic replays cover cross-task success, state-drift refresh with inactive time removed, and ambiguous lookup abort. Run `pnpm recovery:replay`; its output is explicitly prototype-only and is not human-efficiency evidence or formal study-v2 data.
