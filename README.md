...
The companion CLI exposes `object-list`, `object-upsert`, `object-supersede`, `object-retire`, `object-audit`, and `object-archive`. `object-list` reports a non-blocking curation warning at 64 active current-task objects; the existing 256-active limit remains fail-closed. Read-only `object-audit` compares the current task registry with checker-valid stable artifacts and reports active partials, stable overlaps, terminal archive candidates, unmatched terminal records, and ambiguous matches. It explicitly leaves omission measurement to a milestone review because missing-but-needed objects cannot be derived by scanning Chat. After a checker-valid stable artifact has taken over and the task-local object is terminal, explicit `object-archive` writes a private audit copy first and removes the hot duplicate only for one unique same-type/same-name stable match. Active, unmatched, and ambiguous records are never archived. Archived records are not lookup authority; old visible text resolves through the stable artifact. Payloads are strict, bounded JSON and are persisted only under the private Companion state directory, not the project workspace. Registration refreshes the identity-only catalog but never opens a card or creates a Chat Turn.
