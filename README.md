...
2. After an explicit task/workspace binding, the Host projects a bounded identity-only catalog. It contains stable terms and types, not facts, authority locators, sources, or file content. Capacity is allocated in two passes: one highest-priority unambiguous primary term per object first, then aliases with the remaining slots.
...
The live local-workspace index excludes `fixtures/`, `study-dist/`, and `study-release/` by default, so copied research objects cannot suppress or ambiguate current-workspace marks. Fixture Quiet Mode keeps its independent, explicitly labeled data source. Discovery and usable capacity are separate: a small workspace keeps the full compatible file surface, while a large workspace projects the complete development-context surface (source modules, tests, documents, supported configurations, and checked milestone artifacts) instead of an arbitrary first-N truncation. If that development surface itself exceeds the 2,048-entry runtime bound, or discovery exceeds its independent 20,000-file bound, indexing fails closed.
...
Rebinding the same host task, route, scope, and canonical workspace rotates the binding capability while adopting the existing task-object records without changing their entity or content revisions. Records are never adopted across another task, route, scope, or workspace.

The companion CLI exposes `object-list`, `object-upsert`, `object-supersede`, `object-retire`, `object-audit`, `object-review`, and `object-archive`. `object-list` reports a non-blocking curation warning at 64 active current-task objects; the existing 256-active limit remains fail-closed. Read-only `object-audit` compares the current task registry with checker-valid stable artifacts and reports active partials, stable overlaps, terminal archive candidates, unmatched terminal records, and ambiguous matches. Because missing-but-needed objects cannot be derived by scanning Chat, `object-review --review-file <absolute-json>` measures only a bounded list of terms explicitly declared as needed at a stable milestone. It resolves those terms against the current runtime-validated workspace + task-local identity surface and reports available, missing, ambiguous, and type-mismatch counts plus omission and resolution-failure rates. It reads no Provider detail, calls no model, stores no Chat text, and does not persist the review. After a checker-valid stable artifact has taken over and the task-local object is terminal, explicit `object-archive` writes a private audit copy first and removes the hot duplicate only for one unique same-type/same-name stable match. Active, unmatched, and ambiguous records are never archived. Archived records are not lookup authority; old visible text resolves through the stable artifact. Task-object payloads are strict, bounded JSON and are persisted only under the private Companion state directory, not the project workspace. Registration refreshes the identity-only catalog but never opens a card or creates a Chat Turn.

## Private multi-milestone observation

The one-shot `object-review` remains read-only and non-persisting. When an opted-in long task reaches a real stable milestone, `milestone-observe --review-file <absolute-json>` may run that same explicit need review and append one private, task-fenced observation. The ledger stores only SHA-256 fingerprints, result states and types, workspace/task-local source layers, timestamps, and aggregate lifecycle/capacity counts. It never stores raw terms, milestone names, Chat, Provider facts, file content, workspace paths, or Codex task/thread identifiers. Events form a validated hash chain, writes are atomic, and the ledger fails closed at 512 events or 4 MiB.

`milestone-summary` reads only observations for the current task/workspace context and reports aggregate availability, omission, resolution failures, source-layer counts, recurring need fingerprints, and the latest curation snapshot. This private longitudinal signal can reveal repeated missing or ambiguous identities and help calibrate sparse object selection and graduation. It is not a Dashboard, does not create cards or Chat Turns, and is not evidence that people complete work faster.

The `milestone-observe` response also includes `feedback` for only the terms in that explicit current review. It joins each current term to its private digest history and classifies it as first observation, stable available, new gap, recurring gap, changed gap, or recovered. Transition labels compare only with the immediately previous observation for that term: a gap-to-available edge is `recovered` once, while the next available observation is `stable_available`; historical gaps do not make recovery sticky. Raw terms appear only in this ephemeral response (`persisted=false`) because they were already supplied by the caller; they are never added to the ledger. Only a recurring gap may return `review_registration`, and that is a bounded curation prompt—not permission to create an object, artifact, card, or Chat Turn automatically.

A second cross-project replay used every commit in [Vite PR #22642](https://github.com/vitejs/vite/pull/22642). Across the baseline plus 11 commits, task, concept, decision, and change identities appeared at the first stable milestone, one widened change used explicit supersession, and the merged task retired. Merge-main, link-only, and test-skip commits created no new objects. The replay also forced the large-workspace path and verified that a malformed milestone review returns an error without killing the detached companion. These are mechanism and curation results, not human-efficiency evidence or proof that the Vite change passes its own tests.

The first real recovery baseline then declared exactly three resume/handoff/understanding needs and resolved all three without ambiguity: one task-local Task plus two stable workspace artifacts. The ongoing `Long Task Recovery Dogfood` Task graduated to an evidence-bound workspace record. A newly created Codex task then bound the same workspace, opened that Task through the native Chat Lane, and the user confirmed that the card directly answered the goal, current status, completed work, next step, and boundary without inheriting private task-local state. This is one immediate cross-task human acceptance; delayed-return timing, comparative efficiency, and Chat Turn reduction still require later observation.

The developer-only [Recovery Observation Prototype](docs/recovery-observation-prototype.md) now has an explicit opt-in native interaction bridge. A bounded Renderer event protocol reports only interaction type and a task fence; the Host hashes object identity and routes events into an in-memory Recovery Episode only after `recovery-start`. It derives active recovery, card-dwell, navigation, Chat Turn, interaction, wrong-object, refresh, and lookup-failure metrics without persisting Chat text, paths, task fingerprints, or authority detail. Run `pnpm recovery:replay` for deterministic pipeline diagnostics or `pnpm recovery:native-check` for unit plus real Edge Headless interaction checks. This remains prototype-only, is separate from formal study-v2, and is not human-efficiency evidence.

## Observed test execution

`pointable-context-test-execution` creates a Verification only after it actually starts one explicitly declared command with `shell=false`. It binds the process exit, timestamps, command/stdout/stderr SHA-256, and Git snapshots before and after execution. A zero exit becomes PASS only when the workspace snapshot does not drift; non-zero becomes FAIL; timeout, cancellation, output overflow, or revision drift remains inconclusive. Raw command arguments and output are not written into the workspace or private event. The workspace receives one bounded evidence line plus one create-only Verification record, which must pass the existing Record Check. Test source, build success, Chat prose, and unexecuted commands still cannot produce PASS.
...
The historical v2.21 `OpenAI.Codex 26.814.5517.0` / executable `151.0.7922.137` record passed all four automatic and all ten evidence-bound manual gates against renderer digest `616008d2…5154`; it remains historical evidence for v2.21 only. The v2.22 Task Object Registry and card-local text-selection bundle (`6a0aaa90…c6a2`) also independently passed automatic 4/4 plus manual 10/10 and remains historical evidence for that exact bundle. v2.24 remained unqualified after manual acceptance exposed an offscreen-anchor orphan-card defect. v2.25 closed an un-moved card correctly, but refresh qualification then exposed that a deliberately dragged card still disappeared after new Chat output and that alias-heavy catalogs could omit later objects. v2.26 (`f570c4de…843c`) pinned a user-moved card, allocated one primary annotation term per unambiguous object before aliases, and independently passed automatic 4/4 plus manual 10/10 on exact Codex package `26.814.5517.0`. v2.27 (`fbc4d463…c278`) added the read-only curation audit and passed automatic host gates 4/4 with all manual gates pending. v2.28 added explicit milestone object review; v2.29 added reliable test-execution events and its `77d7051e…3814` bundle passed automatic 4/4. v2.30 added the private multi-milestone observation ledger; v2.31 added ephemeral current-review feedback. The v2.32 `p-map` replay made recovery transition-local. The v2.33 bundle `73e78bbc…08d2` added the fail-closed large-workspace development surface and crash-safe malformed-review handling and passed automatic Host gates 4/4 on exact Codex package `26.818.5229.0`; that qualification is historical evidence for v2.33 only. The current v2.34 bundle `e12d713a…dca2` adds the explicit native Recovery Observation bridge. Its targeted 41/41 checks and real Edge Headless interaction stream pass. The exact Codex package `26.818.5229.0` then exposed one qualified main target through the numeric IPv6 loopback endpoint, and the current bundle passed automatic Host gates 4/4; all ten manual interaction gates remain pending and no earlier evidence is inherited. Run the read-only inspector against this exact host and bundle:

```powershell
node dist/src/compatibility/qualification-cli.js --workspace-root . --renderer-bundle host/workspace-companion.mjs --host-version 26.818.5229.0 --json
```
