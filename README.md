...
The first real recovery baseline then declared exactly three resume/handoff/understanding needs and resolved all three without ambiguity: one task-local Task plus two stable workspace artifacts. The ongoing `Long Task Recovery Dogfood` Task is now an evidence-bound workspace record, so a later Codex task can recover it without inheriting private task-local state. This proves structural cross-task availability only; delayed-return comprehension and time savings still require later observation.
