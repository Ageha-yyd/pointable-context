...
The `milestone-observe` response also includes `feedback` for only the terms in that explicit current review. It joins each current term to its private digest history and classifies it as first observation, stable available, new gap, recurring gap, changed gap, or recovered. Raw terms appear only in this ephemeral response (`persisted=false`) because they were already supplied by the caller; they are never added to the ledger. Only a recurring gap may return `review_registration`, and that is a bounded curation prompt—not permission to create an object, artifact, card, or Chat Turn automatically.
...
The historical v2.21 `OpenAI.Codex 26.814.5517.0` / executable `151.0.7922.137` record passed all four automatic and all ten evidence-bound manual gates against renderer digest `616008d2…5154`; it remains historical evidence for v2.21 only. The v2.22 Task Object Registry and card-local text-selection bundle (`6a0aaa90…c6a2`) also independently passed automatic 4/4 plus manual 10/10 and remains historical evidence for that exact bundle. v2.24 remained unqualified after manual acceptance exposed an offscreen-anchor orphan-card defect. v2.25 closed an un-moved card correctly, but refresh qualification then exposed that a deliberately dragged card still disappeared after new Chat output and that alias-heavy catalogs could omit later objects. v2.26 (`f570c4de…843c`) pinned a user-moved card, allocated one primary annotation term per unambiguous object before aliases, and independently passed automatic 4/4 plus manual 10/10 on exact Codex package `26.814.5517.0`. v2.27 (`fbc4d463…c278`) added the read-only curation audit and passed automatic host gates 4/4 with all manual gates pending. v2.28 added explicit milestone object review; v2.29 added reliable test-execution events and its `77d7051e…3814` bundle passed automatic 4/4. v2.30 added the private multi-milestone observation ledger and recorded the first real current-task observation. The current v2.31 product slice adds ephemeral current-review curation feedback and has bundle digest `c9c789a3…1478` on exact Codex package `26.818.5229.0`. After reinstall and exact-task rebinding, the automatic Host gates pass 4/4. A second real current-task observation classified all three repeated available needs as `stable_available`, with no curation action; the private summary now contains two milestones and six available results. All ten manual interaction gates remain pending, so v2.31 is not yet fully build-qualified. No earlier manual evidence is inherited. Run the read-only inspector against this exact host and bundle:
