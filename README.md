...
The historical v2.21 `OpenAI.Codex 26.814.5517.0` / executable `151.0.7922.137` record passed all four automatic and all ten evidence-bound manual gates against renderer digest `616008d2…5154`; it remains historical evidence for v2.21 only. The v2.22 Task Object Registry and card-local text-selection bundle (`6a0aaa90…c6a2`) also independently passed automatic 4/4 plus manual 10/10 and remains historical evidence for that exact bundle. The current v2.24 capacity/archive/rebind bundle has digest `2c46a836…d1e8`; automatic host-contract gates pass 4/4, while all ten manual interaction gates remain pending and cannot inherit v2.22 evidence. Run the read-only inspector with the actual host version and current bundle:
